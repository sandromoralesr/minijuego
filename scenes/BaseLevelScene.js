export class BaseLevelScene extends Phaser.Scene {
  ...
  getDeviceCategory() {
    ...
    if (w < 400) return 'xs';   // iPhone SE, Galaxy A pequeños
    if (w < 600) return 'sm';   // iPhones normales, Galaxy S
    if (w < 900) return 'md';   // iPhones Pro Max, tablets pequeñas
    return 'desktop';
  }
  ...
  getRuntimeLevelConfig() {
    ...
    if (cat === 'desktop') return config;

    const scales = {
      xs: { platform: 0.42, ball: 0.62, speedMult: 0.78, obstacle: 0.85 },
      sm: { platform: 0.52, ball: 0.70, speedMult: 0.85, obstacle: 0.88 },
      md: { platform: 0.62, ball: 0.78, speedMult: 0.90, obstacle: 0.92 },
    };
    ...
    return {
      ...config,
      platformScale: config.platformScale * s.platform,
      ballScale:     config.ballScale     * s.ball,
      ballSpeedMin:  Math.floor(config.ballSpeedMin * s.speedMult),
      ballSpeedMax:  Math.floor(config.ballSpeedMax * s.speedMult),
      obstacleScale: config.obstacleScale * s.obstacle,
    };
  }
  ...
  create() {
    ...
    // Rebota en izquierda, derecha y arriba. No rebota abajo: si cae, pierde.
  ...
  createStateTexts(width, height, isMobile) {
    this.gameoverText = this.add.text(
      ...
      'LA SUERTE SE ESCAPÓ',
      {
        ...
        shadow: { offsetX: 0, offsetY: 0, color: '#ff1744', blur: 22, fill: true }
      }
    ...
    this.winText = this.add.text(
      ...
      'NIVEL SUPERADO',
      {
        ...
        align: 'center',
        wordWrap: { width: width * 0.92 },
        shadow: { offsetX: 0, offsetY: 0, color: '#00e676', blur: 22, fill: true }
      }
    ).setOrigin(0.5).setVisible(false).setDepth(1000);
  ...
  createPlatform(width, height) {
    ...
    this.plataform = this.physics.add.image(width / 2, platformY, 'plataform')
      .setImmovable(true)
  ...
  createObstacles() {
    ...
    const maxPatternWidth = width * {
      xs: 0.94, sm: 0.90, md: 0.86, desktop: 0.58
    }[cat];

    const maxPatternHeight = height * {
      xs: 0.38, sm: 0.40, md: 0.44, desktop: 0.46
    }[cat];

    const baseSpacingX = { xs: 52, sm: 62, md: 72, desktop: 112 }[cat];
    const baseSpacingY = { xs: 50, sm: 58, md: 68, desktop: 102 }[cat];

    const spacingX = Math.min(baseSpacingX, maxPatternWidth  / Math.max(cols - 1, 1));
    const spacingY = Math.min(baseSpacingY, maxPatternHeight / Math.max(rows - 1, 1));
    const spacing  = Math.min(spacingX, spacingY);

    const minSize  = { xs: 44, sm: 56, md: 68, desktop: 105 }[cat];
    const maxSize  = { xs: 72, sm: 84, md: 96, desktop: 150 }[cat];
    const sizeMult = isMobile ? 1.65 : 1.95;

    const obstacleSize = Phaser.Math.Clamp(
      spacing * this.runtimeConfig.obstacleScale * sizeMult,
      minSize,
      maxSize
    );

    const startX = width / 2 - ((cols - 1) * spacing) / 2;
    const startY = { xs: height * 0.13, sm: height * 0.15, md: height * 0.16, desktop: height * 0.15 }[cat];
  ...
  // ─── Cuenta regresiva al siguiente nivel ────────────────────────────────────
